          ...
          //   Determine the source and clean everything up
            ...
            function loadFixtures(keys, callback) {
              async.map(keys, function loadFixture (key, callback2) {
                ...
                fixtures.forEach(function saveKey (fixture, i) {
                  // DEV: Fixtures can be `null` (as in loaded via a placeholder)
                  if (fixture) {
                    fixture.key = keys[i];
                  }
                });
            ...
            return loadSeriesFixtures(function handleFixtures (err, fixtures) {
              ...
              var matchingFixtures = fixtures.filter(function filterTallerFixture (fixture) {
                ...
                // TODO: Handle off by one more accurately
                var len = expectedPastRequestkeys.length - 1;
                for (; i < len; i++) {
              ...
              async.parallel([
                function loadActivePastRequests (callback) {
                  console.log('past request keys', localReqMsg.pastRequestKeys);
                  loadFixtures(localReqMsg.pastRequestKeys, callback);
                },
                function loadSavedPastRequest (callback) {
                  console.log('matched', tallestMatchingFixture.pastRequestKeys);
                ...
                // TODO: Handle `null` scenario for requests
                function infoPluck(info) {
                  if (info) {
                    return info.request;
                  } else {
                    return info;
                ...
                // console.error('Active/saved requests diff:',
                //   jsonDiff.diffString(activeRequests.map(infoPluck), savedRequests.map(infoPluck)));
